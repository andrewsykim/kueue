...
func (w *RayJobWebhook) Default(ctx context.Context, obj runtime.Object) error {
	...
	if isAnElasticJob(rayjob) {
		// Ensure that the PodSchedulingGate is present in the RayJob's pod templates for its Head and all its Workers
		utilpod.GateTemplate(&rayjob.Spec.RayClusterSpec.HeadGroupSpec.Template, kueue.ElasticJobSchedulingGate)

		for index := range rayjob.Spec.RayClusterSpec.WorkerGroupSpecs {
			wgs := &rayjob.Spec.RayClusterSpec.WorkerGroupSpecs[index]
...
// isAnElasticJob returns whether the RayJob is an elastic job
func isAnElasticJob(job *rayv1.RayJob) bool {
...
func (w *RayJobWebhook) validateCreate(ctx context.Context, job *rayv1.RayJob) (field.ErrorList, error) {
	...
	if w.manageJobsWithoutQueueName || jobframework.QueueName(kueueJob) != "" {
		...
		} else if ptr.Deref(clusterSpec.EnableInTreeAutoscaling, false) {
			// Should not use autoscaler unless ElasticJobsViaWorkloadSlices is enabled. Once the resources are reserved by queue the cluster should do its best to use them.
			allErrors = append(allErrors, field.Invalid(clusterSpecPath.Child("enableInTreeAutoscaling"), clusterSpec.EnableInTreeAutoscaling, "a kueue managed job can use autoscaling only when the ElasticJobsViaWorkloadSlices feature gate is on and the job is an elastic job"))
		}
...
func validateElasticJob(job *rayv1.RayJob) field.ErrorList {
	...
	if !slices.Contains(job.Spec.RayClusterSpec.HeadGroupSpec.Template.Spec.SchedulingGates, workloadSliceSchedulingGate) {
		allErrors = append(allErrors, field.Invalid(specPath.Child("headGroupSpec").Child("template").Child("spec").Child("schedulingGates"), job.Spec.RayClusterSpec.HeadGroupSpec.Template.Spec.SchedulingGates, "an elastic job must have the ElasticJobSchedulingGate"))
	}
